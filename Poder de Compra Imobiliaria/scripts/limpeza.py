...
def preprocessar(df_inpc, df_imoveis):
    ...
    df_imoveis["mes"] = pd.to_datetime(df_imoveis["mes_raw"], format="%b-%y", errors="coerce")
    ...
    df = pd.merge(df_inpc, df_imoveis, on="mes", how="inner").sort_values("mes").reset_index(drop=True)
